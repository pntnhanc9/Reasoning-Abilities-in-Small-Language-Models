...
class HuggingFaceModelGenerator(ModelGenerator):
    """Planner: local model generator using HuggingFace Transformers (cyankiwi/Qwen3.5-9B-AWQ-4bit).

    Supports fully offline/on-device GPU inference:
    - First run: downloads weights to HuggingFace cache (~/.cache/huggingface/hub)
    - Subsequent runs (or with local_model_path): loads directly from disk, no internet needed
    """

    DEFAULT_HF_MODEL = "cyankiwi/Qwen3.5-9B-AWQ-4bit"
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """Initialize the Planner model generator.

        Args:
            hf_model_id: HuggingFace model ID (used when local_model_path is None).
            local_model_path: Path to a local directory containing the model weights.
                If set, loads from disk without any network request.
            local_files_only: If True, raises an error instead of downloading.
                Useful to guarantee offline operation after first download.
            use_reasoning: Whether to prepend reasoning text before the code block.
            include_feedback: Whether to include error traceback feedback in prompts.
        """
        ModelGenerator.__init__(self, **kwargs)
        self.use_reasoning = use_reasoning
        self.include_feedback = include_feedback

        # Resolve the model source: local path takes priority over HF hub ID
        model_source = local_model_path if local_model_path is not None else hf_model_id
        self.hf_model_id = model_source
        ...
        logger.info(f"Loading Planner on GPU: {model_source}" +
                    (" [OFFLINE/local]" if local_model_path or local_files_only else " [HuggingFace Hub]"))

        load_kwargs = dict(
            local_files_only=local_files_only or (local_model_path is not None),
        )
        self.tokenizer = AutoTokenizer.from_pretrained(model_source, **load_kwargs)
        # device_map="auto" distributes layers across available GPUs automatically
        ...
        self.model.eval()
        logger.info(f"Planner loaded on: {next(self.model.parameters()).device}")
    ...
    def load_from(existing_model_dir: str, new_model_dir: str, **kwargs):
        ...
        return HuggingFaceModelGenerator(model_dir=new_model_dir, **kwargs)
    ...
    def generate_model(
        ...
        with open(completions_path, "w") as f:
            f.write(json.dumps({"response_text": response_text}, indent=4))
    ...
    def format_example_model(model_data: Dict, use_reasoning: bool = True):
        ...
        text += HuggingFaceModelGenerator.embed_code(model_data["model_source"])
        return text
